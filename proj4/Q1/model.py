...
class Gaussians:
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        # When isotropic, the s1,s2,s3 that forms the diagonal scale matrix are equal (say s)
        # So SS^T becomes |s|**2*Identity, further since R is orthogonal RR^T becomes identity. 
        # Hence, covaraince becomes s^2 X I  
        ...
        # HINT: You can use a function from pytorch3d to convert quaternions to rotation matrices.
            ...
            R=transforms.quaternion_to_matrix(quats)  # N,3,3
    ...
    def compute_cov_2D(
        ...
        # of this function?
        T = camera.get_world_to_view_transform().get_matrix()
        W = T[:,:3,:3]  # (N, 3, 3)
    ...
    def evaluate_gaussian_2D(points_2D: torch.Tensor, means_2D: torch.Tensor, cov_2D_inverse: torch.Tensor):
        ...
        A = torch.einsum('ndk,nhd->nhk', cov_2D_inverse, delta)
        q = torch.einsum('nhd,nhd->nh', delta, A)
        power = -0.5*q  # (N, H*W)
...
class Scene:
    ...
    def get_idxs_to_filter_and_sort(self, z_vals: torch.Tensor):
        ...
        idxs_valid = torch.nonzero(valid, as_tuple=False).squeeze(-1)
        depths_valid = z_vals[idxs_valid]

        order = torch.argsort(depths_valid, dim=0, stable=True)
        idxs = idxs_valid[order] 
    ...
    def compute_transmittance(
        ...
        ### YOUR CODE HERE ###
        # HINT: Refer to README for a relevant equation.
        # [S, S*(1-alpha_0), S*(1-alpha_0)*(1-alpha_1), ...]
        transmittance = torch.cumprod(one_minus_alphas, dim=0)  # (N+1, H, W)
        transmittance = transmittance[:alphas.shape[0]]  # (N, H, W)
